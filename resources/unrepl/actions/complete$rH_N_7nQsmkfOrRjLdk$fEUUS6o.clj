(ns
 unrepl.actions.complete$rH_N_7nQsmkfOrRjLdk$fEUUS6o
 (:require
...
(defn complete
  ...
  [left right ns-name]
  (if-some [[_ partial-string] (re-matches #"(?s)^(?:[^\"\\;]*|\\.|;[^\n]*?\n|\"(?:[^\"\\]|\\.)*\")*(\"[^\"\\]*)" left)]
    (complete-path (edn/read-string (str partial-string \")))
    (let [[_ left prefix] (re-matches #"(?s)(.*?)([^\s,\";@^`~()\[\]{}]*)" left)
          [_ suffix right] (re-matches #"(?s)([^\s,\";@^`~()\[\]{}]+)?(.*)" right)
